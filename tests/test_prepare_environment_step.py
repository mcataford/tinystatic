...
from tinystatic.steps.prepare_environment_step import InvalidConfigFileException, MissingConfigFileException, PrepareEnvironmentStep
...
def mock_config_dict():
    return {"paths": { "static_path": "static", "content_path": "content", "templates_path":
        "templates" }}
@pytest.fixture
...
def test_honors_working_directory_overrides_via_cli_context(tmpdir, mock_config, mock_config_dict, prepare_env_step):
...
def test_outputs_project_root_and_config_file(tmpdir, prepare_env_step, mock_config, mock_config_dict):
    config_file = Path(tmpdir, 'site_config.toml')
